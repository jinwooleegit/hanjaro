...
// 한자 데이터 파일 읽기
try {
  const data = JSON.parse(fs.readFileSync('data/hanja_database_fixed.json', 'utf8'));
  
  // 7단계(중학교 1학년) 찾기
  const level7 = data.hanja_database.basic.levels.find(level => level.name.includes('7단계'));
  
  // 설명 변경 - 50자에서 100자로
  if (level7) {
    level7.description = '중학교 1학년 과정에서 배우는 한자 100자';
    
    // 새로운 한자 추가 (51번부터 100번까지)
    const newCharacters = [
      {
        "character": "功",
        "meaning": "공 공",
    ...
    // 새 한자 추가
    level7.characters = [...level7.characters, ...newCharacters];
    
    // 파일 저장
    fs.writeFileSync('data/hanja_database_fixed.json', JSON.stringify(data, null, 2));
    console.log('중학교 1학년(7단계) 한자가 50자에서 100자로 확장되었습니다.');
  } else {
    console.error('7단계(중학교 1학년) 한자를 찾을 수 없습니다.');
  }
} catch (error) {
